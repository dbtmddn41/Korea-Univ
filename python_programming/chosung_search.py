...
def chosung_indexing(cho_indexing, filename):
    ...
    with open(filename) as fin:
        for line in fin.readlines():
            ...
            cho = str2chostr(word)
            if cho not in cho_indexing:
                cho_indexing[cho] = []
            cho_indexing[cho].append((word, int(freq)))
    for value in cho_indexing.values():
        value.sort(key = lambda x: (x[1], x[0]*(-1)), reverse=True)
