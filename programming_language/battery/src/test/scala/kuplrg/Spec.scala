...
class Spec extends SpecBase {
  ...
  test(
    eval("""
    def applyTwice(f: Number => Number, x: Number): Number =
      f(f(x))
    val square = (n: Number) => n * n
    applyTwice(square, 2)
    """),
    "16: Number",
  )
  ...
  test(
    eval("""
    def isEven(n: Number): Boolean =
      if (n == 0) true else isOdd(n - 1)
    def isOdd(n: Number): Boolean =
  ...
  test(
    ...
    def getOrElse(opt: Option, default: Number): Number = opt match {
      case Some(v) => v
      case None() => default
    }
    getOrElse(None(), 100)
  ...
  test(
    ...
    def length(lst: List): Number = lst match {
      case Nil() => 0
  ...
  test(
    eval("""
    exit[String]("This is an exit expression")
    """),
    "ERROR: String",
  )
  ...
  test(
    eval("""
    def makeAdder(n: Number): Number => Number = (x: Number) => x + n
  ...
  test(
    eval("""
    enum Tree {
      case Node(value: Number, left: Tree, right: Tree)
      case Empty()
    }
    def height(t: Tree): Number = t match {
      case Empty() => 0
      case Node(_, l, r) => 1 + (
        if (height(l) > height(r)) height(l)
        else height(r)
      )
    }
    height(Node(1, Node(2, Empty(), Empty()), Empty()))
    """),
    "2: Number",
  )
  ...
  test(
    ...
    compose[Number,Number,Number](double, inc)(5)
    """),
    "12: Number",
  )
  ...
  test(
    ...
    def curry[A,B,C](f: (A, B) => C): A => B => C =
      (x: A) => (y: B) => f(x, y)
    val add = (x: Number, y: Number) => x + y
    curry[Number,Number,Number](add)(3)(4)
    """),
    "7: Number",
  )
  ...
  test(
    eval("""
    def repeat(n: Number): String => String =
      (s: String) => if (n <= 0) "" else s ++ repeat(n-1)(s)
    repeat(3)("ha")
    """),
    "\"hahaha\": String",
  )
  ...
  test(
    eval("""
    enum Result {
      case Success(value: Number)
      case Error(message: String)
    }
    def safeDiv(x: Number, y: Number): Result =
  ...
  testExc(
    ...
    def unbox[T,U](b: Box[T]): U = b match {
      case Box(x) => x
    }
    """)
  )
  ...
  test(
    eval("""
    lazy val x: Number = y + 1
    lazy val y: Number = z + 2
    lazy val z: Number = 3
    x
    """),
    "6: Number",
  ...
  test(
    eval("""
    def memoize(f: Number => Number): Number => Number = {
      enum Cache {
        case Empty()
        case Entry(key: Number, value: Number, rest: Cache)
      }
      def lookup(cache: Cache, k: Number): Number = cache match {
        case Empty() => 0
        case Entry(key, value, rest) =>
          if (key == k) value else lookup(rest, k)
      }
      def cached(n: Number, cache: Cache): Number = {
        val result = f(n)
        Entry(n, result, cache)
        result
      }
      (n: Number) => cached(n, Empty())
    }
  ...
  test(
    eval("""
    def forall[T](p: T => Boolean): (T => Boolean) => T => Boolean =
      (q: T => Boolean) => (x: T) => !p(x) || q(x)
    val isPositive = (n: Number) => n > 0
    val isEven = (n: Number) => n % 2 == 0
    forall[Number](isPositive)(isEven)(2)
    """),
    "true: Boolean",
  )
  ...
  test(
    ...
    def map[L,R,T](e: Either[L,R], f: R => T): Either[L,T] = e match {
      case Left(l) => Left[L,T](l)
      case Right(r) => Right[L,T](f(r))
    }
    map[String,Number,Number](Right[String,Number](42), (x: Number) => x * 2) match {
      case Left(s) => 0
      case Right(n) => n
    }
    """),
