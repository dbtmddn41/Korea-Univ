...
class Spec extends SpecBase {
  ...
  test(eval("var x = 1; var y = 2; x + y"), "3")
  test(eval("var x = 10; while(x > 5) x -= 1; x"), "5")
  test(eval("var x = 0; var y = 1; while(x < 3) { y *= 2; x += 1; }; y"), "8") 
  test(eval("var error = 3; try { throw (error); 42 } catch(e) { e }"), "3")
  test(eval("var f = x => x * 2; f(21)"), "42")
  test(eval("(x =>* { yield x; yield x+1; })(1).next()"), "{ value: 1, done: false }")
  test(eval("var x = 1; var y = 2; y = x++; x"), "2")
  testExc(eval("break"))
  testExc(eval("continue"))
  test(eval("1 + 2 * 3"), "7")
  test(eval("var f = (x,y) => x + y; f(2,3)"), "5")
  // Additional tests for eval (continued)
  test(eval("var x = true; x && false"), "false")
  test(eval("var x = false; x || true"), "true")
  test(eval("var x = 5; --x; x"), "4")
  test(eval("var x = 5; x--; x"), "4")
  test(eval("function square(x) { return x * x; } square(5)"), "25")
  test(eval("var x = 0; while (x < 5) { x++; }; x"), "5")
}
