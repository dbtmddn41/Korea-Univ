...
_ENGKEY2JAMO_ = {'r': 'ㄱ', 'R': 'ㄲ', 's': 'ㄴ', 'e': 'ㄷ', 'E': 'ㄸ', 'f': 'ㄹ', 'a': 'ㅁ', 'q': 'ㅂ', 'Q': 'ㅃ', 't': 'ㅅ', 'T': 'ㅆ', 'd': 'ㅇ', 'w': 'ㅈ', 'W': 'ㅉ', 'c': 'ㅊ', 'z': 'ㅋ', 'x': 'ㅌ', 'v': 'ㅍ', 'g': 'ㅎ', 'k': 'ㅏ', 'o': 'ㅐ', 'i': 'ㅑ', 'O': 'ㅒ', 'j': 'ㅓ', 'p': 'ㅔ', 'u': 'ㅕ', 'P': 'ㅖ', 'h': 'ㅗ', 'hk': 'ㅘ', 'ho': 'ㅙ', 'hl': 'ㅚ', 'y': 'ㅛ', 'n': 'ㅜ', 'nj': 'ㅝ', 'np': 'ㅞ', 'nl': 'ㅟ', 'b': 'ㅠ', 'm': 'ㅡ', 'ml': 'ㅢ', 'l': 'ㅣ', 'rt': 'ㄳ', 'sw': 'ㄵ', 'sg': ' ㄶ', 'fr': 'ㄺ', 'fa': 'ㄻ', 'fq': 'ㄼ', 'ft': 'ㄽ', 'fx': 'ㄾ', 'fv': 'ㄿ', 'fg': 'ㅀ', 'qt': 'ㅄ'}

_DOUBLE_MOEM_ = {'ㅗㅏ': 'ㅘ', 'ㅗㅐ': 'ㅙ', 'ㅗㅣ':'ㅚ', 'ㅜㅓ': 'ㅝ', 'ㅜㅔ': 'ㅞ', 'ㅜㅣ': 'ㅟ', 'ㅡㅣ': 'ㅢ'}

_DOUBLE_JAEM_ = {'ㄱㅅ': 'ㄳ', 'ㄴㅈ': 'ㄵ', 'ㄴㅎ': 'ㄶ', 'ㄹㄱ': 'ㄺ', 'ㄹㅁ': 'ㄻ', 'ㄹㅂ': 'ㄼ', 'ㄹㅅ': 'ㄽ', 'ㄹㅌ': 'ㄾ', 'ㄹㅍ': 'ㄿ', 'ㄹㅎ': 'ㅀ', 'ㅂㅅ': 'ㅄ'}
###############################################################################
...
def jamo2engkey(jamo_str):
    engkey = ''
    for letter in jamo_str:
        if letter in _JAMO2ENGKEY_:
            engkey += _JAMO2ENGKEY_[letter]
        else:
            engkey += letter
    return engkey


def engkey2jamo(key_str):
    han_lett = ''
    for letter in key_str:
        if letter in _ENGKEY2JAMO_:
            han_lett += _ENGKEY2JAMO_[letter]
        else:
            han_lett += letter
    return han_lett
...
def count_jung(s):
    if is_jung(s[0]):
        if len(s) > 1:
            if is_jung(s[1]):
                if s[:2] in _DOUBLE_MOEM_:
                    return 2, _DOUBLE_MOEM_[s[:2]]
        return 1, s[0]
    else:
        return 0, None
...
def count_jong(s):
    ...
    for i in range(2):
        ...
        if is_jung(s[i+1]):
            ...
            break
    if num == 2:
        if s[:2] in _DOUBLE_JAEM_:
            return 2, _DOUBLE_JAEM_[s[:2]]
        else:
         return 1, s[0]
    return num, s[:num]
...
def jamo2syllable(s):
    ...
    while i < len_str:
        ...
        if s[i] not in _CHO_ or len_str-1 <= i:
            syllable += s[i]
            i += 1
            continue
        cho = s[i]
        ...
        jong = ''
        if i < len_str:
            jong_num, jong = count_jong(s[i:])
            if jong_num != 0:
                i += jong_num

        cho, jung, jong = find_index(cho, jung, jong)
        syllable += compose(cho, jung, jong)
