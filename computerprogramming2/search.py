...
def get_highlight(query, sentence):
    ...
    query = query.split(' ')
    for word in query:
        high = '<font color="blue"><b>' + word + '</b></font>'
        sentence = sentence.replace(word, high)
    return sentence
...
def search( inverted_indexing, sentences, query):
    ...
    query = query.split(' ')
    sens = set()
    if query[0] in inverted_indexing:
        sens = inverted_indexing[query[0]]
    for word in query[1:]:
        if word in inverted_indexing:
            sens &= inverted_indexing[word]
    sens_list = list(sens)
    sens_list.sort()
    return sens_list
